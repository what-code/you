(function() {
    ...
    $('form.search-form').each(function() {
        ...
        form.find('.sub').click(function(e) {
            e.preventDefault();
            ...
            //edit by duanyu
            if($(this).attr("id") != "sub_btn_map"){
            	form.submit();
            }
        });

         /*keywords.on('keyup.key',function(e){ 
            if(e.keyCode ==13){                                 
    ...
    $('.cityselect').on('click', 'a', function(e) {
        ...
        if ($(this).hasClass('close')) {
            ...
            //add by duanyu s
            $(this).parents('.rows:first').find('input[name=selectedCityId]').val($(this).attr('data-city'));
            try{
            	$('#innerSelectedCityId').val($(this).attr('data-city'));
            }catch(e){}
            
            //“全部”的时候，搜索框的样式,长的
            if($(this).attr('data-city') == '-1'){
            	//$('#hotel_keyword_text').attr('class','');
            	//$('#keyword_inner_text').attr('class','filter-text l ml10 J-tip');
            	//$('#div_sub_btn_map').hide();
            }else{
            	//$('#hotel_keyword_text').attr('class','filter-text l ml10 filter-text2 J-tip');
            	//$('#keyword_inner_text').attr('class','filter-text l ml10 filter-text-short J-tip');
            	//$('#div_sub_btn_map').show();
            }
            //add by duanyu e
            citySelect.hide();
...
function setSearch() {
    ...
    $('#j_s_way .has-more .more').on('click', function() {
        if (!s) {
            $(this).text('收起').parent('.has-more').css({
                height: 'auto'
            }).end().addClass('more_up');
            s = 1;
        } else {
            $(this).text('更多').parent('.has-more').css({
                height: ''
            }).end().removeClass('more_up');
            s = 0;
        }
        return false;
    });
    ...
    $('#j_s_way .rt a').on('click', function() {
        ...
        if (!$(this).hasClass('act')) {
            ...
            //add by duanyu start 搜索结果页提交事件（days）
            $("#currPageNo").val(1);
            //console.log($("#J_order").serialize());
            //console.log(window.location.href,$("#rootPath").val()+ $("#J_order").attr('action') + '?' +$("#J_order").serialize());
    ...
    $('.hotel-cat:last').css('border-bottom','none');
    
    $('.J-tip').focus(function(){
    ...
    $('.hotel-tip,.J-tip').on('click', function(e) {
        e.stopPropagation();
    });
