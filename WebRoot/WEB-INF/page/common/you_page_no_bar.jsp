		...
		<div>
			<c:choose>
				<c:when test="${pageList.totalPages>0}">
					<!-- 首页 -->
					<input type="hidden" id="pager_text" value="${rootPath}/${pagerUrl}">
					<c:choose>
						<c:when test="${searchDto.currPageNo == 1}"><a href="javascript:void('0');" class="first dis">首页</a></c:when>
						<c:otherwise><a href="${rootPath}/${pagerUrl}1${fromType == 'list' ? '.html' : ''}" class="first">首页</a></c:otherwise>
					</c:choose>
					<!-- 上一页 -->
					<c:choose>
						<c:when test="${pageList.hasPrevPage}"><a class="prev" href="${rootPath}/${pagerUrl}${searchDto.currPageNo-1}${fromType == 'list' ? '.html' : ''}">&lt;</a></c:when>
					...
					<c:choose>
						...
						<c:otherwise><a href="${rootPath}/${pagerUrl}1${fromType == 'list' ? '.html' : ''}">1</a></c:otherwise>
					</c:choose>
					...
					<c:if test="${searchDto.currPageNo >= 3}">
						<c:choose>
							<c:when test="${searchDto.currPageNo == 3}">
								<a href="${rootPath}/${pagerUrl}2${fromType == 'list' ? '.html' : ''}">2</a>
							</c:when>
							<c:when test="${searchDto.currPageNo > 3}">
								<a href="${rootPath}/${pagerUrl}${searchDto.currPageNo - 2}${fromType == 'list' ? '.html' : ''}">${searchDto.currPageNo - 2}</a>
								<a href="${rootPath}/${pagerUrl}${searchDto.currPageNo - 1}${fromType == 'list' ? '.html' : ''}">${searchDto.currPageNo - 1}</a>
							</c:when>
							<c:otherwise>
							</c:otherwise>
						</c:choose>
					...
					<c:if test="${pageList.totalPages != 1 }">
						<c:choose>
							...
							<c:otherwise>
								<a href="${rootPath}/${pagerUrl}${pageList.totalPages }${fromType == 'list' ? '.html' : ''}">${pageList.totalPages}</a>
							</c:otherwise>
						</c:choose>
					</c:if>
					<!-- 下一页 -->
					<c:choose>
						<c:when test="${pageList.hasNextPage}">
							<a class="next"
								href="${rootPath}/${pagerUrl}${searchDto.currPageNo+1}${fromType == 'list' ? '.html' : ''}">&gt;</a>
						</c:when>
						<c:otherwise>
					...
					<c:choose>
						...
						<c:otherwise>
							<a href="${rootPath}/${pagerUrl}${pageList.totalPages }${fromType == 'list' ? '.html' : ''}"
								class="last">尾页</a>
						</c:otherwise>
					</c:choose>
