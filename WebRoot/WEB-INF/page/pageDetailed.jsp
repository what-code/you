	...
	<div class="w980 clear-fix">
	    ...
	    <div class="crumb clearfix">
	        <div class="inner">
				...
				<c:if test="${menuCurPageTag=='hotel' }">
					<a href="${rootPath}/list_1_-1_1.html">酒店</a>
				</c:if>
						...
						<li>
							<c:choose>
								<c:when test="${user_session_login == 'true' && user_session_login != null }">
									<a href="${youGoods.sourceUrl }" rel="nofollow" target="_blank" cps="true"><img src="${gl }" alt="${youGoods.name}"></a>
								</c:when>
								<c:otherwise>
									<a href="${youGoods.sourceUrl }" class="tankuang" href="javascript:void(0);" rel="nofollow" target="_blank"
										clickUrl="${youGoods.clickUrl}" sourceUrl="${youGoods.sourceUrl}" cps="true"><img src="${gl }" alt="${youGoods.name}"></a>
								</c:otherwise>
							</c:choose>
						</li>
					</c:forEach>
				...
				<c:when test="${menuCurPageTag!='hotel' }">
					<div class="show-info" data-id="${youGoods.id }">
						...
						<div class="now">
							...
							<a class="tankuang btn" href="${youGoods.sourceUrl}" clickUrl="${youGoods.clickUrl}" rel="nofollow" sourceUrl="${youGoods.sourceUrl}" target="_blank" cps="true">开始预订</a>
						</div>
					</div>
				</c:when>
				<c:otherwise>
       ...
       <div class="module">
           <div class="module-hd clearfix">
				...
				<c:if test="${menuCurPageTag=='abroadTravel'  || (menuCurPageTag=='searchresult' && youGoods.youType=='1')}">
					<b class="t2"></b>
					<a class="more" href="${rootPath}/travel" target="_blank">更多<span>&gt;</span></a>
				</c:if>
				...
				<c:if test="${menuCurPageTag=='hotel' }">
					<b class="t4"></b>
					<a class="more" href="${rootPath}/list_1_-1_1.html"
						target="_blank">更多<span>&gt;</span></a>
				</c:if>
			</div>
					...
					<div class="mod" data-id="${gl.id }" data-url="${gl.sourceUrl }"
						...
						<div class="tit">
							<c:choose>
								...
								<c:otherwise>
									<a class="tankuang" href="${gl.sourceUrl}" href="${gl.sourceUrl} target="_blank"
										clickUrl="${gl.clickUrl}"  rel="nofollow" sourceUrl="${rootPath}/detail_${searchDto.searchType}_${gl.id }.html">【${dataSource[gl.source]!=null?dataSource[gl.source]:gl.source}】  ${gl.name }</a>
								</c:otherwise>
							</c:choose>
						</div>
						<div class="pic">
							<c:choose>
								<c:when
									...
									<a href="${gl.sourceUrl}" rel="nofollow" target="_blank" cps="true"><span><img src="http://staticcdn.b5m.com/static/images/common/transparent.png" style="background:url(${gl.imgurl }) no-repeat center center" alt="${gl.name }"></span></a>
								</c:when>
								<c:otherwise>
									<a class="tankuang" href="${gl.sourceUrl}" target="_blank"
										clickUrl="${gl.clickUrl}" rel="nofollow" sourceUrl="${rootPath}/detail_${searchDto.searchType}_${gl.id }.html" cps="true"><span><img src="http://staticcdn.b5m.com/static/images/common/transparent.png" style="background:url(${gl.imgurl }) no-repeat center center" alt="${gl.name }"></span></a>
								</c:otherwise>
							</c:choose>
							<div class="original">
								<p>
						...
						<div class="opt">
							...
							<c:choose>
								<c:when
									test="${user_session_login == 'true' && user_session_login != null }">
									<a href="${gl.sourceUrl}" rel="nofollow" target="_blank" class="btn" cps="true">去抢购</a>
								</c:when>
								<c:otherwise>
									<a class="tankuang btn" href="${gl.sourceUrl}" rel="nofollow"
										target="_blank" clickUrl="${gl.clickUrl}"
										sourceUrl="${gl.sourceUrl}" cps="true">去抢购</a>
								</c:otherwise>
							</c:choose>
						</div>
					</div>
