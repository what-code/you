...
<c:if test="${menuCurPageTag=='hotel' }">
	<title>${youGoods.name }_帮5游</title>
	<meta name="keywords" content="${youGoods.name }">
	<meta name="description" content="${youGoods.brandInfo }">
</c:if>
<c:if test="${menuCurPageTag!='hotel' }">
	<title>${youGoods.name }_帮5游</title>
	<meta name="keywords" content="${youGoods.name }">
	<meta name="description" content="${youGoods.name}">
</c:if>
...
<body>
	        ...
	        <div class="inner">
				...
				<c:choose>
					<c:when test="${menuCurPageTag=='hotel' }">
						<a href="${rootPath}/list_1_${searchDto.selectedCityId}_1.html">酒店</a>
					</c:when>
					<c:otherwise>
						<a href="${rootPath}/travel">旅游</a>
					</c:otherwise>
				</c:choose>
	        ...
	        <div data-id="${youGoods.id }" id="J_carousel" class="pic-show">
			            <ul class="big-pic">
							<c:forEach var="gl" items="${fn:split(youGoods.b5mImgUrls, ',')}" varStatus="status">
								<li>
									<a class="cps" href="javascript:;" clickUrl="${youGoods.clickUrl}" rel="nofollow"><img src='${fn:trim(gl) == "" ? "../images/you/188x118.jpg" : gl}' alt="${youGoods.name}" onerror="this.src='${rootPath }/images/you/430x258.jpg';"></a>	
								</li>
							</c:forEach>
			            </ul>
			                ...
			                <ul class="sma-pic">
			                	<c:forEach var="gl" items="${fn:split(youGoods.b5mImgUrls, ',')}" varStatus="status">
									<li ${status.count==1?"class='on'":'' }>
										<a class="cps" href="javascript:;" clickUrl="${youGoods.clickUrl}"><img src='${fn:trim(gl) == "" ? "../images/you/188x118.jpg" : gl}' onerror="this.src='${rootPath }/images/you/430x258.jpg';"></a>
									</li>
								</c:forEach>
			                </ul>
			            </div>	            	
					...
					<div class="show-info" data-id="${youGoods.id }">
						...
						<ul class="list">
							...
							<c:if test="${!fn:contains(youGoods.groupDate,'天天出发')}">
								<li><label>发团日期</label>${fn:length(youGoods.groupDate) > 40 ? fn:substring(youGoods.groupDate,0,40) : youGoods.groupDate}${fn:length(youGoods.groupDate) > 40 ? "...":""}</li>
							</c:if>
							<c:if test="${fn:contains(youGoods.groupDate,'天天出发')}">
						...
						<div class="now">
							...
							<a class="tankuang btn cps" href="javascript:;" clickUrl="${youGoods.clickUrl}" rel="nofollow" sourceUrl="${youGoods.sourceUrl}">开始预订</a>
						</div>
					</div>
				</c:when>
				<c:otherwise>
					<div class="show-info" data-id="${youGoods.id }">
						...
						<ul class="list">
							...
							<c:if test="${youGoods.buildings!=null && fn:trim(youGoods.buildings) != ''}">
								<li><label>周边地标</label>
								<c:choose>
										<c:when test="${fn:length(youGoods.buildings) > 32}">
											${fn:substring(youGoods.buildings, 0, 31)}...
										</c:when>
										<c:otherwise>
											${youGoods.buildings}
										</c:otherwise>
					...
					<table>
						...
						<c:forEach var="gl" items="${hotelDetailList}" varStatus="status">
							<tr class="no-top-border">
								<td class="bed-type"><a href="${youGoods.sourceUrl}" target="_blank"><img src="${gl.imgurl }" style="display:${gl.imgurl==''||gl.imgurl==null?'none':'' }" class="bed-type-img" alt="${gl.roomType }"></a><a href="${youGoods.sourceUrl}" target="_blank">${gl.roomType }</a></td>
								<td class="bed-list">
									<div class="bed-item bed-item-no-bottom-border">
										...
										<div class="bed-info">
											￥<a href="${youGoods.sourceUrl}" target="_blank"><span class="red-light">${gl.price }</span></a>
										</div>
									</div>
								</td>
							</tr>
	...
	<!--detail e-->
	<div style="margin-top:10px;" class="wp">
			 <!-- 广告位：帮5游右上方广告位_详情页中部_980*90 -->
			<script type="text/javascript" >BAIDU_CLB_SLOT_ID = "850024";</script>
       ...
       <div class="module">
           <div class="module-hd clearfix">
				...
				<c:choose>
					<c:when test="${menuCurPageTag=='hotel' }">
						<b class="t4"></b>
						<a class="more" href="${rootPath}/list_1_${searchDto.selectedCityId}_1.html"
							target="_blank">更多<span>&gt;</span></a>
					</c:when>
					<c:otherwise>
						<b class="t3"></b>
						<a class="more" href="${rootPath}/travel"
							target="_blank">更多<span>&gt;</span></a>
					</c:otherwise>
				</c:choose>
			</div>
			<div class="module-bd module-bd-detail clearfix">
				<c:forEach var="gl" items="${pageList.all}" varStatus="status">
					<div class="mod" data-id="${gl.id }" data-url="${gl.sourceUrl }"
						...
						<div class="tit">
							<a href="${rootPath}/detail_${searchDto.searchType}_${gl.id }.html" rel="nofollow" target="_blank">【${dataSource[gl.source]!=null?dataSource[gl.source]:gl.source}】   ${gl.name }</a>
						</div>
						<div class="pic">
							<a href="${rootPath}/detail_${searchDto.searchType}_${gl.id }.html" rel="nofollow" target="_blank"><span><img src="http://staticcdn.b5m.com/static/images/common/transparent.png" style="background:url(${gl.imgurl }) no-repeat center center" alt="${gl.name }"></span></a>
							<div class="original">
								<p>
						...
						<div class="opt">
							<span>&yen;<c:if test="${gl.salesPrice > 0 }"><em>${gl.salesPrice }</em>起</c:if><c:if test="${gl.salesPrice <= 0 || fn:trim(gl.salesPrice)== ''}"><em>暂无报价</em></c:if>
							</span>
